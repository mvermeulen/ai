...
1. **Crossover Point & Scaling:**
   * **Scalar Crossover & Overhead Realities (Large Sweep):** While small-scale tests of 10M numbers suggest a crossover point near Block 1024 ($2^{42}$) with only a $-1.7\%$ delta, full-block benchmarks at Block 100,000 ($2^{48.6}$) reveal a different behavior. When executing a full block sweep (1 Billion starting numbers), the vanilla scalar CPU with Domain-Switching actually exhibits a **$-10.3\%$ slowdown** (throughput drops from 11.52 M/s to 10.33 M/s). This is because the average trajectory length at Block 100,000 is still short enough that the overhead of entering/exiting the $n+1$ domain, checking `max_safe_k` tables, and executing 128-bit multiplications dominates the execution time of the brief 128-bit segment before transitioning to native 64-bit loops. Domain-switching only becomes a net speedup at even higher ranges, such as Block 1,000,000 ($2^{52}$), where trajectories spend enough steps in the 128-bit space to amortize this startup overhead.
