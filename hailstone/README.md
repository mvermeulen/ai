...
3. **Suffix-First Search (Apriori Cutoffs)**: By generating unique `fpoly` suffix equivalence classes and applying even-class exclusion and modulo 6 filtering, the search is restructured to execute only the non-redundant allowed suffixes. Suffix-First search is enabled by default with width 20 on CPU, HIP, and Vulkan backends, pruning the checked search space by **85.17%** and yielding up to **3.23x CPU speedup**, **2.45x GPU HIP speedup**, and **56x GPU Vulkan speedup**.
