...
TEST_CASE("Tiebreaker breaks ties using group and head-to-head criteria", "[Tiebreaker]") {
    ...
    // Team 2: 2 wins, 0 draws, 1 loss = 6 points, GF: 10, GA: 2 (GD: +8)
    t2.addWin(); t2.addWin(); t2.addLoss();
    t2.addGoalsFor(10); t2.addGoalsAgainst(2);
    ...
    REQUIRE(sorted[0]->abbreviation() == "ARG"); // 6 pts, +3 GD, H2H Win
    REQUIRE(sorted[1]->abbreviation() == "FRA"); // 6 pts, +8 GD
    REQUIRE(sorted[2]->abbreviation() == "KOR"); // 3 pts, -3 GD, H2H Win
    REQUIRE(sorted[3]->abbreviation() == "RSA"); // 3 pts, -2 GD
}
