...
void AsciiPrinter::printAllStandings(const Tournament &tournament,
                                     const MatchSimulationResults *simResults) {
  ...
  for (const auto &group : groups) {
    ...
    for (size_t i = 0; i < standings.size(); ++i) {
      ...
      std::cout << "│ " << std::left << std::setw(12) << team->fullName()
                ...
                << std::setw(4) << team->goalsFor() << " │ " << std::left
                << std::setw(12) << status << " │\n";
    }
    std::cout << "└──────────────┴──────┴──────┴──────┴──────────────┘\n\n";
  }
...
static std::string formatTeamName(const std::string &teamCode) {
  if (teamCode.empty())
    return "";
  if (teamCode.find("WINNER_GROUP_") == 0)
    return "1" + teamCode.substr(13);
  if (teamCode.find("RUNNER_UP_GROUP_") == 0)
    return "2" + teamCode.substr(16);
  if (teamCode.find("THIRD_GROUP_") == 0)
    return "3rd";
  if (teamCode.find("R32_WINNER_") == 0)
    return "W" + teamCode.substr(11);
  if (teamCode.find("R16_WINNER_") == 0)
    return "W" + teamCode.substr(11);
  if (teamCode.find("QUARTERFINAL_WINNER_") == 0)
    return "W" + teamCode.substr(20);
  if (teamCode.find("SEMIFINAL_WINNER_") == 0)
    return "W" + teamCode.substr(17);
  if (teamCode.find("TBD_") == 0)
    return "TBD";
  if (teamCode.length() > 4)
    return teamCode.substr(0, 3);
  return teamCode;
}
...
static double computeWinProb(const Team &home, const Team &away,
                             const MonteCarlo *mc) {
  if (!mc)
    return 0.5;
  double eloDiff = home.eloRating() - away.eloRating();
  double homeBoost =
      MonteCarlo::isHost(home.abbreviation()) ? mc->hostAdvantage() : 0.0;
  double awayBoost =
      MonteCarlo::isHost(away.abbreviation()) ? mc->hostAdvantage() : 0.0;

  double lambdaHome =
      mc->baseRate() * std::exp(mc->alpha() * eloDiff + homeBoost);
  double lambdaAway =
      mc->baseRate() * std::exp(-mc->alpha() * eloDiff + awayBoost);
...
void AsciiPrinter::printBracket(const Tournament &tournament, bool unplayedOnly,
                                const MonteCarlo *mc) {
  ...
  for (size_t r = 0; r < rounds.size(); ++r) {
    for (size_t i = 0; i < rounds[r].size(); ++i) {
      ...
      if (r == 4 && teamStrHome == "TBD")
        teamStrHome = "W101";
      if (r == 4 && teamStrAway == "TBD")
        teamStrAway = "W102";
      ...
      if (!match.isPlayed() && mc) {
        ...
        if (homeTeamPtr && awayTeamPtr) {
          ...
          int awayP = 100 - homeP;
          std::string probStr =
              std::to_string(homeP) + "/" + std::to_string(awayP) + "%";
      ...
      if (r == 4 && match.isFinal() && !unplayedOnly) {
        std::string winner =
            match.homeTeamWon() ? match.homeTeam() : match.awayTeam();
  ...
  std::vector<std::string> headers = {"ROUND OF 32 ", "ROUND OF 16 ",
  ...
  for (int L = 0; L < 63; ++L) {
    ...
    if (!emptyLine && hasVisibleCol) {
      ...
      // Check if entirely empty lines can be skipped, but to maintain visual
      // tree, we should print them Actually, we must print empty lines if they
      // are within the bracket range to preserve vertical scale.
      std::cout << "\n";
    }
  }
  std::cout << "\n";
...
void AsciiPrinter::printSimulationResults(
    const MatchSimulationResults &results) {
  struct StageInfo {
    std::string header;
    const std::map<std::string, double> *probs;
  };
  std::vector<StageInfo> stages = {{"Adv R32 ", &results.r32Probability},
  ...
  if (firstUnresolvedStageIdx >= 0) {
    for (size_t i = static_cast<size_t>(firstUnresolvedStageIdx);
         i < stages.size(); ++i) {
  ...
  std::sort(activeTeams.begin(), activeTeams.end(),
...
void AsciiPrinter::printImpactAnalysis(const ImpactAnalysisResults &analysis) {
  if (analysis.gameImpacts.empty()) {
    std::cout << "No upcoming unplayed matches found for impact analysis."
              << std::endl;
    return;
  }
  ...
  for (const auto &imp : analysis.gameImpacts) {
    ...
    std::cout << "│ " << std::left << std::setw(6) << imp.matchId << " │ "
              << std::setw(10) << imp.date << " │ " << std::right
              << std::setprecision(1) << std::fixed
              << (imp.homeDeltaR32 >= 0 ? "+" : "") << std::setw(10)
              << imp.homeDeltaR32 * 100.0 << "%"
              << " │ " << (imp.awayDeltaR32 >= 0 ? "+" : "") << std::setw(10)
              << imp.awayDeltaR32 * 100.0 << "%"
